...
v2f vert (appdata v) 
{
    ...
    o.uv.xy = v.texcoord0.xy * _MainTex_ST.xy + _MainTex_ST.zw;
		...
		half3 normal_WS = (mul((half3x3)_Object2World, v.normal));		
	#endif
			...
			half3 ligthDir_WS = _WorldSpaceLightPos0.xyz - mul(_Object2World, v.vertex).xyz;
		#endif
	#endif
	    ...
	    half3 refl_WS = mul ((float3x3)_Object2World, viewRefl);
	#endif
		...
		float4 TtoW0 = float4(mul(rotation, _Object2World[0].xyz), worldRefl.x)*1.0;
	    float4 TtoW1 = float4(mul(rotation, _Object2World[1].xyz), worldRefl.y)*1.0;
		float4 TtoW2 = float4(mul(rotation, _Object2World[2].xyz), worldRefl.z)*1.0;
	#endif
